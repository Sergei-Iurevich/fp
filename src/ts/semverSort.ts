export function semverSort(argArray: string[]): string[] {
  const inArray = [...argArray];

  function compareElemArray(
    argA: number | undefined,
  ...
  function compareVersion(argA: string, argB: string) {
    ...
    const detectedIndex = strArrayA.findIndex(
      (item, index) => compareElemArray(item, strArrayB[index]) !== 0
    );
